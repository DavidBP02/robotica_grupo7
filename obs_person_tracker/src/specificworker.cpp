...
{
    auto tupla = room_detector.compute_features(helios, scene);
    auto vector_tuples_first_pos= get<0>(tupla);

    vector<QLineF> lines;
    for(auto& r : vector_tuples_first_pos){
        lines.push_back(r.second);
    }
